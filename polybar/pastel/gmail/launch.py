...
parser.add_argument('-l', '--label', default='CATEGORY_PERSONAL')
...
def update_count(count_was):
    ...
    sum_categorized = sum([gmail.users().labels().get(userId='me', id=ID).execute()["messagesUnread"] for ID in ["CATEGORY_SOCIAL", "CATEGORY_PROMOTIONS", "CATEGORY_FORUMS"]])
    count = inbox['messagesUnread'] - sum_categorized
    print_count(count)
